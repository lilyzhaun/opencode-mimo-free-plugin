...
当本包发布到 npm 后，在 OpenCode 配置文件（`opencode.json` 或 `opencode.jsonc`）中添加插件：
...
在 npm 发布前，可以先构建插件并把产物复制到 OpenCode 全局插件目录：
...
OpenCode 启动时会自动加载 `~/.config/opencode/plugins/` 下的 JavaScript 文件。
...
2. 如果 npm 包已发布，在 `"plugin"` 数组中添加 `"opencode-mimo-free-plugin"`。如果数组不存在则创建。

3. 如果 npm 包尚未发布，请改用本地全局插件安装方式：
   ```bash
   bun install
   bun run build
   mkdir -p ~/.config/opencode/plugins
...
4. 可选：设置 `"model": "mimo/mimo-auto"` 将 MiMo Auto 作为默认模型。
...
5. 不需要任何 API Key 或环境变量。插件会自动处理认证。

6. 重启 OpenCode（或开始新会话）。模型 `mimo/mimo-auto` 将出现在模型列表中。

7. 验证方法：运行 `opencode models`，确认 `mimo/mimo-auto` 已列出。
